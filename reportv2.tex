...
novel multi-size clique dataset, explanation F1 stays between 0.13 and
0.26 across clique sizes and the optimizer never recovers a full clique.
Second, motivated by 2026 work showing that jointly-trained
\emph{self-explainable} GNNs can produce explanations provably unrelated
to a model's actual reasoning \citep{azzolin2026gnnexplanations}, we add
two constant, label-independent color features to the clique dataset and
test whether \GNNEx's feature mask mistakes either one for a real signal.
We find a partial version of this failure: one color feature shows almost
no class-dependent gap, but the other shows a large one, disproportionate
to the underlying difference in classifier accuracy between classes.
\end{abstract}
...
We train a 2-layer GCN \citep{kipf2017semi} with 64 hidden units, ReLU
activations, and dropout ($p=0.5$), for 600 epochs using Adam
\citep{kingma2015adam} with learning rate $10^{-2}$ and weight decay
$5\times10^{-5}$. We apply class-weighted cross-entropy
($w_1 = n_{\mathrm{base}}/n_{\mathrm{clique}}$) to compensate for the 127:73
class imbalance. These hyperparameters were chosen to keep the trained
classifier reasonably balanced across both classes, since a poorly-tuned
network risks under-fitting the base class entirely and confounding the
explanation results that follow (Section~\ref{sec:limitations}).
...
(Eq.~\ref{eq:feat_mask}), one row per node in $v$'s computation graph. Let
$\bfw_v \in (0,1)^2$ be the sigmoid of $v$'s row of $\bfM_F^{(v)}$: the
weight \GNNEx assigns to $v$'s $\text{color}_1$ and $\text{color}_2$
entries. A faithful explainer should assign $\bfw_v$ a low value that does
not depend on $v$'s class. A class-dependent asymmetry,
...
The GCN reaches 78.5\% overall accuracy: 79.5\% on base nodes and 76.7\% on
clique nodes, a balanced fit that gives the explainer a real gradient
signal on both classes.

Table~\ref{tab:results} shows explanation quality still falls well short of
full recovery at every clique size, though not strictly monotonically: F1
is 0.199 ($k=3$), 0.261 ($k=4$), and 0.132 ($k=5$), and the optimizer never
recovers the full clique on average, at best 2.56 of 4 edges. The $k=5$
case remains clearly the worst, consistent with the local-optima mechanism
of Section~\ref{subsec:localopt}: as $k$ grows, more competing sub-clique
local optima exist. The non-monotonic gap between $k=3$ and $k=4$ likely
reflects the small number of cliques per size (8 and 6) rather than the
effect's absence.
...
\begin{table}[t]
  ...
  \begin{tabular}{lcccc}
    ...
    \midrule
    $k=3$ (triangles)  & 0.202 & 0.326 & $0.199 \pm 0.134$ & 1.96 / 3 \\
    $k=4$              & 0.286 & 0.329 & $0.261 \pm 0.124$ & 2.56 / 4 \\
    $k=5$              & 0.156 & 0.117 & $0.132 \pm 0.110$ & 1.87 / 5 \\
    All clique nodes   & 0.216 & 0.272 & $0.201 \pm 0.134$ & -- \\
    \bottomrule
  \end{tabular}
\end{table}
...
receive $\text{color}_1=0.230$ and $\text{color}_2=0.572$, and non-clique
nodes receive $\text{color}_1=0.209$ and $\text{color}_2=0.291$, so the
default regularization does not drive a genuinely irrelevant feature to 0
within 150 steps.

The class-dependent gap is uneven across the two features: $\text{color}_1$
still shows only a small gap (0.230 vs.\ 0.209), but $\text{color}_2$ shows
a substantial one (0.572 vs.\ 0.291), with clique nodes, the very slightly
\emph{harder} class here (76.7\% vs.\ 79.5\% accuracy), receiving the much
higher weight. This direction fits a compensation account, though a
2.8-point accuracy gap cannot explain a 0.281 weight gap on its own.
\GNNEx does not fabricate a clean, deterministic encoding
($g(\text{color}_1){=}\text{clique}$): 0.572 is still far from 1. But this
large, disproportionate gap on $\text{color}_2$ is a clearer instance of
the Section~\ref{subsec:anchor} failure on a reasonably well-balanced
classifier, a gap Section~\ref{sec:limitations} attributes to
$\Phi$'s training, not \GNNEx.
...
reflects the classifier, not the explainer. The clique GCN of
Section~\ref{sec:experiments} reaches a comparatively balanced 76.7\%
accuracy on clique nodes and 79.5\% on base nodes, giving the explainer a
genuine gradient signal on both classes, but some of the residual F1 gap
may still not be \GNNEx's fault. The same applies even more sharply to our
...
understood as a heuristic, not a provable bound. We then concretely investigate the local-optima behavior the
original authors already anticipate but do not characterize, confirmed on
the clique benchmark: F1 stays between 0.13 and 0.26 across clique sizes,
worst at $k=5$, and the optimizer never recovers a full clique. Then we adapt a 2026 unfaithfulness result for
self-explainable GNNs into a testable question for \GNNEx: once the anchor
features are made truly constant, its feature mask still cannot reproduce
the clean, deterministic failure of a jointly-trained extractor, but one of
the two anchor features receives a large, class-dependent weight gap out of
proportion to the underlying classifier's accuracy gap.

Future work could address the mistake found in the derivation, it might be
interesting to characterize its badness or to even investigate the false 
convexity assumption in more detail, perhaps considering
whether there are distinguishable classes where this derivation works provably,
or in general investigate which GNNs would fulfill the convexity assumption.
...
\texttt{gnnexplainer.py} and available at
\url{https://github.com/ArionScheid/Seminar-in-AI.git}. Random seeds are
fixed throughout for reproducibility. The \GNNEx implementation in PyTorch Geometric requires
