...
class imbalance. These hyperparameters were tuned by a small grid search
over depth, width, epochs, and weight decay for the best-balanced accuracy
across classes; a 3-layer, 300-epoch configuration under-fit the base class
(Section~\ref{sec:limitations}).
...
the Section~\ref{subsec:anchor} failure than with the earlier, less
balanced classifier, a gap Section~\ref{sec:limitations} attributes to
...
reflects the classifier, not the explainer. An earlier, untuned
configuration of our clique GCN reached 90.4\% accuracy on clique nodes but
only 60.6\% on base nodes; the tuned GCN of Section~\ref{sec:experiments}
narrows this to 76.7\% and 79.5\%, but some of the residual F1 gap may
still not be \GNNEx's fault. The same applies even more sharply to our
...
worst at $k=5$, and the optimizer never recovers a full clique. Our main
empirical contribution adapts a 2026 unfaithfulness result for
